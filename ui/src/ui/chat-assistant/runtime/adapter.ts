...
function makeCustomMetadata(part: AssistantMessagePart): { custom: CustomMetadata } {
  return {
    custom: {
      openclawPart: part,
    },
...
function toThreadMessage(part: AssistantMessagePart): ThreadMessage {
  if (part.type === "divider") {
    return {
      ...
      metadata: makeCustomMetadata(part),
  ...
  return {
    ...
    createdAt: new Date(part.timestamp),
    metadata: makeCustomMetadata(part),
    ...(role === "assistant" ? { status: { type: "complete", reason: "stop" as const } } : {}),
  } as unknown as ThreadMessage;
}
