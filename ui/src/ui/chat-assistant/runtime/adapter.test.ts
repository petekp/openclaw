...
describe("assistant runtime adapter", () => {
  ...
  it("reports running state while sending or streaming", () => {
    const sendingAdapter = createAssistantChatRuntimeAdapter(
      createProps({
        sending: true,
      }),
    );
    const streamingAdapter = createAssistantChatRuntimeAdapter(
      createProps({
        stream: "partial",
      }),
    );
    const idleAdapter = createAssistantChatRuntimeAdapter(createProps());

    expect(sendingAdapter.isRunning).toBe(true);
    expect(streamingAdapter.isRunning).toBe(true);
    expect(idleAdapter.isRunning).toBe(false);
  });

  it("calls onAbort from onCancel only when abort is enabled", async () => {
    const onAbortAllowed = vi.fn();
    const allowedAdapter = createAssistantChatRuntimeAdapter(
      createProps({
        canAbort: true,
        onAbort: onAbortAllowed,
      }),
    );
    await allowedAdapter.onCancel?.();
    expect(onAbortAllowed).toHaveBeenCalledTimes(1);

    const onAbortBlocked = vi.fn();
    const blockedAdapter = createAssistantChatRuntimeAdapter(
      createProps({
        canAbort: false,
        onAbort: onAbortBlocked,
      }),
    );
    await blockedAdapter.onCancel?.();
    expect(onAbortBlocked).not.toHaveBeenCalled();
  });

  it("does not send new messages when runtime is disabled", async () => {
    const onDraftChange = vi.fn();
    const onSend = vi.fn();
    const adapter = createAssistantChatRuntimeAdapter(
      createProps({
  ...
  it("resolves reload prompt using wrap-around search when needed", async () => {
    const onReloadMessage = vi.fn();
    const baseTime = Date.now();
    const adapter = createAssistantChatRuntimeAdapter(
      createProps({
        ...
        messages: [
          ...
          {
            role: "user",
            content: [{ type: "text", text: "wrap prompt" }],
            timestamp: baseTime + 1_000,
          },
        ],
      }),
    ...
    const parentId = adapter.messages[0]?.id ?? null;
    await adapter.onReload?.(parentId, {
      parentId,
      sourceId: "source-wrap",
    } as unknown as NonNullable<Parameters<NonNullable<typeof adapter.onReload>>[1]>);
