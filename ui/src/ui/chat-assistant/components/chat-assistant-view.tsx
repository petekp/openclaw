...
function MarkdownMessagePart({ text }: TextPartProps) {
  return (
    <div
      ...
      // Sanitization is centralized in toSanitizedMarkdownHtml (DOMPurify + markdown escaping).
...
function openImageUrlInNewTab(url: string) {
        ...
        <div className="chat-message-images">
          {images.map((image, index) => (
            <img
              ...
              onClick={() => openImageUrlInNewTab(image.url)}
            />
          ))}
        </div>
      ) : null}
...
function CompactionIndicator({ status }: { status: CompactionIndicatorStatus | null | undefined }) {
  ...
  useEffect(() => {
    if (!status?.completedAt || status.active) {
      return;
    }
    const remaining = COMPACTION_TOAST_DURATION_MS - (Date.now() - status.completedAt);
    if (remaining <= 0) {
      return;
    }
    const timer = setTimeout(() => forceUpdate((n) => n + 1), remaining);
    return () => clearTimeout(timer);
  }, [status?.completedAt, status?.active]);
...
export function ChatAssistantView({ props }: { props: ChatProps }) {
  const historyThreadMessages = useMemo(
    () =>
      buildAssistantThreadMessages({
        ...props,
        stream: null,
    ...
    [props.sessionKey, props.messages, props.toolMessages, props.showThinking],
  );
  const streamThreadMessages = useMemo(() => {
    if (props.stream === null) {
      return [];
    }
    return buildAssistantThreadMessages({
      ...props,
      messages: [],
      toolMessages: [],
      stream: props.stream,
  ...
  }, [props.sessionKey, props.stream, props.streamStartedAt]);
  const threadMessages = useMemo(
    () => [...historyThreadMessages, ...streamThreadMessages],
    [historyThreadMessages, streamThreadMessages],
  );
  ...
  const renderContextValue = useMemo(
    ...
    [props.onOpenSidebar, showReasoning, props.assistantName, assistantAvatar],
  );
              ...
              <>
                ...
                <div className="chat-sidebar">
                  <MarkdownSidebar
                    ...
                    onClose={props.onCloseSidebar ?? NOOP}
                    onOpenSidebar={props.onOpenSidebar}
                  />
                </div>
              </>
